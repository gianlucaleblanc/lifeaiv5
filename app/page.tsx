...
// ─────────────────────────────────────────────────────────────
// Sign-in nudge modal
// Shown after the user generates their first plan (non-blocking).
// ─────────────────────────────────────────────────────────────
function SignInNudgeModal({
  open,
  onClose,
  onSignIn,
  afterCalendar,
  ...
  onClose: () => void;
  onSignIn: () => void;
  afterCalendar: boolean; // true = user just added to calendar, false = after first generate
}) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-0 sm:p-4">
      <div className="w-full sm:max-w-sm rounded-t-3xl sm:rounded-3xl bg-white shadow-[0_24px_64px_rgba(0,0,0,0.18)] overflow-hidden">
        {/* Pink accent bar */}
        <div className="h-1 w-full bg-gradient-to-r from-[var(--lifeos-pink)] via-[var(--lifeos-pink)]/60 to-transparent" />

        <div className="px-6 pt-6 pb-5">
          {/* Icon */}
          <div className="mb-4 h-11 w-11 rounded-2xl bg-[var(--lifeos-pink)]/10 flex items-center justify-center">
            <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5" stroke="var(--lifeos-pink)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {/* Headline */}
          <div className="text-[18px] font-black text-black" style={{ letterSpacing: "-0.03em" }}>
            {afterCalendar ? "Save your plan" : "Don't lose your plan"}
          </div>

          {/* Subtext */}
          <div className="mt-1.5 text-[13px] text-black/50 leading-relaxed">
            {afterCalendar
              ? "Sign in to sync your calendar across devices and keep your schedule safe."
              : "Create a free account to save this plan, sync across devices, and access it anywhere."}
          </div>

          {/* Sign in button */}
          <button
            onClick={onSignIn}
            className="mt-5 w-full flex items-center justify-center gap-2.5 rounded-2xl bg-black py-3 px-4 text-[14px] font-bold text-white hover:bg-black/80 active:scale-[0.98] transition-all"
          >
            {/* Google logo */}
          ...
          <button
            onClick={onClose}
            className="mt-3 w-full text-center text-[12px] text-black/35 font-medium hover:text-black/55 transition-colors py-1"
          >
            {afterCalendar ? "Skip for now" : "Maybe later"}
          </button>
        </div>
      </div>
    </div>
...
export default function GeneratePage() {
  ...
  function confirmPlanImport() {
    ...
    // Show sign-in nudge if user isn't signed in yet
    if (!user) {
  ...
  return (
    ...
    >
      ...
      {/* ── Sign-in nudge modal ── */}
      <SignInNudgeModal
        open={showSignInNudge && !user}
        onClose={() => setShowSignInNudge(false)}
        onSignIn={() => { setShowSignInNudge(false); void signInWithGoogle(); }}
        afterCalendar={nudgeAfterCalendar}
      />
    </div>
    </>
  );
