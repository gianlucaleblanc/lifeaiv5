    ...
    >
      ...
      <div className="rounded-2xl bg-white border border-black/[0.06] p-5">
        ...
        <div className="space-y-1">
          ...
          {/* Cloud sync / Sign-in row */}
          <div className="flex items-center gap-4 rounded-xl px-4 py-3 hover:bg-black/[0.03] transition-colors">
            <span className="text-xl flex-shrink-0">☁️</span>
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold text-black/80">
                {user ? "Cloud sync" : "Sign in to sync"}
              </div>
              <div className="text-xs text-black/40">
                {user
                  ? `Syncing as ${user.email}`
                  : "Back up your data across devices"}
              </div>
            </div>
            {user ? (
              <button
                onClick={() => { signOut(); toast("Signed out", "info"); }}
                className="text-xs text-black/40 hover:text-black/70 transition-colors px-3 py-1.5 rounded-lg hover:bg-black/[0.05] font-semibold flex-shrink-0"
              >
                Sign out
              </button>
            ) : (
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-semibold text-[var(--lifeos-pink)] px-3 py-1.5 rounded-lg bg-[var(--lifeos-pink)]/10 hover:opacity-80 transition-opacity flex-shrink-0"
              >
